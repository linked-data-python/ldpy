...
| 027 | [027-prefixe-sans-objet-dexecution.md](027-prefixe-sans-objet-dexecution.md) | Les issues au manque le plus attesté de l'étude : `@prefix` ne produit aucune valeur Python. Héritage des préfixes par un graphe DÉSIGNÉ + `@prefix ex: <…> as EX .` pour garder l'objet ; préfixes connus à l'exécution et `DefinedNamespace` écartés et documentés comme frontières | implémenté — livré en ldpy 0.6.3, portée et STRLANG/STRDT depuis |
| 028 | [028-espace-des-graphes-nommes.md](028-espace-des-graphes-nommes.md) | `Dataset` / graphes nommés : l'écriture passe par `@graph expr`, l'interrogation par `s{ … }` ; mesuré — 270 sélecteurs sur 7 870 visent un `Dataset`, dont 32 seulement lisent un quadruplet : pas de forme de lecture quadruplet | tranché — pas de forme quadruplet, la mesure ne la justifie pas |
